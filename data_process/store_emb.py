...
import os
import numpy as np
user_embedding = tf.Variable(tf.random_normal([179086, 32], stddev=0.01), name='user_embedding')
item_embedding = tf.Variable(tf.random_normal([30621, 32], stddev=0.01), name='item_embedding')
user_social_embedding = tf.Variable(tf.random_normal([179086, 32], stddev=0.01), name='user_social_embedding')
item_social_embedding = tf.Variable(tf.random_normal([30621, 32], stddev=0.01), name='item_social_embedding')
...
model_dir = 'BeiBei_disbpr_reg0.01_lr0.001_epoch0+1000_dim32_fs-W-g.ckpt'
checkpoint = 'pretrain/BeiBei/{}'.format(model_dir)
...
np.savez('embedding/BeiBei/{}.npz'.format(model_dir), user_embedding=user_embedding, item_embedding=item_embedding, \
                                                        user_social_embedding=user_social_embedding, item_social_embedding=item_social_embedding)
np.savez('embedding/BeiBei/{}_W.npz'.format(model_dir), W0_kernel=W[0][0], W0_bias=W[0][1], W1_kernel=W[1][0], W1_bias=W[1][1])
print('save done')