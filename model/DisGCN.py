...
class disgcn(base_model):
    ...
    def Y(self, u, f, i, keepdims=True):
        return tf.reduce_sum(u*i, -1, keepdims=keepdims) + tf.reduce_sum(f*i, -1, keepdims=keepdims)
    ...
    def calc_social_loss(self, user_social_list, item_social_list):
        ...
        for user_social, item_social in zip(user_social_list, item_social_list):
            ...
            social_loss += tf.reduce_sum(tf.reduce_mean(tf.nn.softplus(ufi_neg_i-ufi_pos_i), -1)) + \
                tf.reduce_sum(tf.reduce_mean(tf.nn.softplus(ufi_neg_f-ufi_pos_f), -1))
    ...
    def constructTrainGraph(self):
        ...
        cor_social_embedding = tf.gather(tf.concat([user_social, self.item_social_embedding], 0), self.cor_idx)
        cor_interest_embedding = tf.gather(tf.concat([user_int, self.item_embedding], 0), self.cor_idx)
